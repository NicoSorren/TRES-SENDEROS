...
class ProductManager:
    ...
    def add_product(self):
        ...
        costo           = st.number_input("Costo de KG o UNIDAD (en $)", min_value=0, format="%d", key="costo_input")
        factor          = st.number_input("Factor de venta (ej. 1.5 = +50%)", min_value=0.0, value=1.50, step=0.1, format="%.2f", key="factor_input")
        ...
        stock_option    = st.selectbox("Stock disponible?", ["SI", "NO"], key="stock_input")
        stock_str       = "-" if stock_option == "SI" else "0"

        # — Elección de categoría (existente o nueva) —
        cat_option = st.radio("¿Categoría existente o nueva?", ["Existente", "Nueva"], key="cat_option")
        if cat_option == "Nueva":
            nueva_categoria = st.text_input("Nombre de la Nueva Categoría", key="nueva_categoria")
            tipo_categoria  = st.radio("La nueva categoría almacenará productos de…", ["KG", "UNIDAD", "AMBOS"], key="nueva_cat_tipo")
            categoria       = nueva_categoria
        else:
            nueva_categoria = None
            tipo_categoria  = None
            opciones_cat    = self.df["CATEGORIA"].astype(str).str.strip().drop_duplicates().tolist()
            opciones_cat.sort()
            categoria       = st.selectbox("Selecciona la Categoría", opciones_cat, key="categoria_existente")

        # — Elección de subcategoría según modo —
        if cat_option == "Existente":
            sub_existentes = (
                self.df[self.df["CATEGORIA"].astype(str).str.strip() == categoria]
                ["SUBCATEGORIA"]
            ...
            sub_existentes.sort()
            sub_option = st.radio("Subcategoría existente o nueva?", ["Existente", "Nueva"], key="sub_option")
            if sub_option == "Existente":
                subcategoria = st.selectbox("Selecciona subcategoría existente", sub_existentes, key="subcategoria_existente")
            else:
                subcategoria = st.text_input("Nombre de la nueva Subcategoría", key="subcategoria_nueva")
        else:
            # Si la categoría es nueva, pedimos directamente el nombre de la subcategoría
            subcategoria = st.text_input("Nombre de la Subcategoría", key="subcategoria_nueva")

        # — Formulario principal para capturar el resto de datos —
        with st.form(key="add_product_form", clear_on_submit=False):
            tipo     = st.selectbox("Tipo de Venta", ["KG", "UNIDAD"], key="tipo_venta")
            if tipo == "KG" and cat_option == "Existente":
                opciones_frac = (
                    self.df[self.df["CATEGORIA"].
                            astype(str).str.strip() == categoria]
                        ["FRACCIONAMIENTO"]
                        .fillna("")
                        .astype(str)
                        .unique()
                ...
                # si hay varias, puedes ordenarlas o mantener el orden original
                variante = st.selectbox(
                    "Fraccionamiento",
                    options=opciones_frac,
                    index=0,
                    key="fraccionamiento_input"
                )
            else:
                variante = ""
            ...
            submitted = st.form_submit_button("Agregar Producto", disabled=not all_valid)

        if submitted:
            st.session_state.confirm_add_step = True

        # — Fase 2: confirmación e inserción definitiva —
        if st.session_state.confirm_add_step:
            ...
            st.markdown(f"- **Subcategoría:** {subcat_str}")
            ...
            with st.form(key="confirm_add_form", clear_on_submit=False):
                confirm     = st.radio("¿Estás seguro de agregar este producto?", ["SI", "NO"], key="confirm_add")
            ...
            if confirm_btn:
                if confirm == "SI":
                    ...
                    dup = (
                        ...
                        & (df_current["FRACCIONAMIENTO"].fillna("").astype(str).str.strip().str.lower() == variante_str.lower())
                    )
                    if dup.any():
                        st.warning(f"El producto '{producto_str}' en '{categoria_str}' ya existe.")
                    else:
                        # Generar SKU incluyendo subcategoría
                        sku = generar_sku(
                        ...
                        # Insertar en el DataFrame manteniendo orden alfabético dentro de la categoría
                        ...
                        if cat_indices:
                            ...
                            st.session_state.df = pd.concat([before, pd.DataFrame([new_product]), after], ignore_index=True)
                        else:
                            st.session_state.df = pd.concat([df_current, pd.DataFrame([new_product])], ignore_index=True)

                        # Limpieza de formatos
                        self.df = st.session_state.df
                        self.df["COSTO"]  = self.df["COSTO"].apply(lambda v: float(str(v).replace(".", "").replace(",", ".")) if v else 0.0)
                        ...
                        st.success(f"Producto '{producto_str}' agregado correctamente con SKU {sku}.")
        ...
        with st.form(key="delete_product_form"):
            ...
            if submitted:
                if not productos_a_eliminar:
                    ...
                    st.session_state.df.loc[indices, "ACTIVO"] = "No"
                    # Reflejar en la instancia
                    self.df = st.session_state.df
                    st.success("Productos desactivados correctamente.")
                    st.dataframe(self.df)